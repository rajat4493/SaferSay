...
describeIfDb("Postgres live tenant brand", () => {
  ...
  it("returns null before any brand is saved, then round-trips the exact object afterward", async () => {
    ...
    const brand: BrandTheme = {
      ...
      fontFamily: "georgia",
      presetId: "calm-teal",
      introMessage: "We value your honest feedback.",
      completionMessage: "We'll share what we heard within two weeks.",
    };
  ...
  it("a second save overwrites the first, not merges", async () => {
    ...
    await identity.setBrand(tenant.id, {
      name: "First",
      tagline: "A",
      logoDataUrl: null,
      accentColor: "#111111",
      fontFamily: "inter",
      presetId: "calm-teal",
      introMessage: "First intro",
      completionMessage: "First completion",
    });
    await identity.setBrand(tenant.id, {
      name: "Second",
      tagline: "B",
