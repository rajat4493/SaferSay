...
- Enforce k at the reporting/query layer: no segment or report renders below the tenant's threshold; sub-threshold cells are suppressed/rolled up. (Already partly built at k≥5 — make the value a guarded setting.)
...
- Confidentiality invariant sits above the whole grid: **every role sees aggregates ≥k only, scoped to their slice; nobody sees individuals; Company sees no content at all.**
