...
Title/subtitle: **"How we protect your team's answers"** / "Plain-English explanation for HR leads, DPOs, and employee representatives." — retitled from "Security & Confidentiality" to name the actual audience (coherence-directive follow-up). `ConfidentialitySeal` at top, then a **`CopyLinkButton`** (`src/components/CopyLinkButton.tsx`, new, reusable) — `.btn-primary`, copies `window.location.href` to the clipboard and shows a "Link copied." toast, since the point of this page is that an HR admin can forward it directly to a DPO or works-council rep rather than re-explaining the architecture themselves. Then a 2-column grid of six plain-English control cards (Identity store / Response store / Minimum group size / Reminder isolation / Payment isolation / No emotion inference — each a one-line plain-English claim about the architecture).

**Coherence-directive changes:** (1) this page used to also carry a "Production safety status" card (runtime-mode text + a "Check go-live readiness" link + `/privacy`/`/dpa` links) — that entire card is now removed, not just the go-live link Fix A took out earlier. It was operator-facing content (Supabase/Stripe/Resend config state) that had no business on a page now explicitly aimed at DPOs and HR, and its one remaining useful link (go-live) already lives at `/console/readiness` (owner-gated, §3.9). (2) No nav change — the seal strip's "How it works →" link (visible on every `/app/*` screen) is this page's real entry point; it doesn't strictly need to stay a Workspace nav item for that reason. **Flagging, not implementing:** whether to remove "Security" from the Workspace nav list is a call for you — it's low-risk either way since the seal strip link keeps it reachable regardless.
